...
<?php
$table = $prefix . 'logs';
$sql   = $mysqli->query("SELECT id, ip, date, browser, browser_code, os, os_code, country, country_code, type FROM `$table` WHERE type='SQLi' ORDER by id DESC");
while ($row = mysqli_fetch_assoc($sql)) {
    ...
    if (get_banned($row['ip']) == 1) {
        echo '
										    <a href="sqli-logs.php?unban-ip=' . $row['ip'] . '" class="btn btn-flat btn-success"><i class="fas fa-ban"></i> Unban</a>
									        ';
    } else {
        echo '
										    <a href="sqli-logs.php?ban-ip=' . $row['ip'] . '" class="btn btn-flat btn-warning"><i class="fas fa-ban"></i> Ban</a>
									        ';
    }
    echo '
