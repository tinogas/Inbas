...
while ($row = mysqli_fetch_assoc($sql)) {
    ...
    if (get_banned($row['ip']) == 1) {
        echo '
										    <a href="all-logs.php?unban-ip=' . $row['ip'] . '" class="btn btn-flat btn-success"><i class="fas fa-ban"></i> Unban</a>
									        ';
    } else {
        echo '
										    <a href="all-logs.php?ban-ip=' . $row['ip'] . '" class="btn btn-flat btn-warning"><i class="fas fa-ban"></i> Ban</a>
									        ';
    }
    echo '
